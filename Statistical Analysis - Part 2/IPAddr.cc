...
{
    ip_addr = this->getParentModule()->getIndex();      // Set ip address of host according to it's module index
}
...
void IPAddr::handleMessage(cMessage *msg)
{
    ...
    IPv4Msg* IP_pck;
    AppMsg* App_pck;
    if (strcmp(arrive->getName(),"app$i") == 0 ){            // If message came from "app" module create ip header encapsulate and send to "eth"
        AppMsg* rcv_app_pck = check_and_cast<AppMsg*>(msg);
        int dest_ip = 0;
        IP_pck = generateMessage(dest_ip,ip_addr);           // Destination ip is Host '0'
        IP_pck->encapsulate(rcv_app_pck);
        send(IP_pck,"eth$o");
    }
    if (strcmp(arrive->getName(),"eth$i") == 0 ){           // If message came from "eth" module decapsulate and send to "app"
