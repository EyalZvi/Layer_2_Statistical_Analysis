...
{   // Initialization function
    num_of_req = 0;                                         // Set ARP requests to 0
    src_MAC = this->getParentModule()->getIndex();
        ...
        {                                               // THEN Generate EtherType: ARP Reply
            if(rcv->getDest_ip() == my_ip)
            {
                ...
                if(this->getParentModule()->getIndex() == 0 ){
                    ...
                    {
                        num_of_req++;
                        emit(interarrivalTimeSignals[0], delay);
                    }
                    else
                    {
                        emit(interarrivalTimeSignals[1], delay);
                    }

                    last_pck_arrival[rcv->getSrc_ip()] = simTime();

                    if(num_of_req == int(par("test_arp_requests")))
                        endSimulation();
                }
                //
